...
surface_t::surface_t(unsigned int w, unsigned int h, texture_ptr_t texture) : _framebuffer(0), _width(w), _height(h) {
...
surface_t::~surface_t() { glDeleteFramebuffers(1, &_framebuffer); }

surface_ptr_t surface_t::create(unsigned int w, unsigned int h) { return surface_ptr_t(new surface_t(w, h, nullptr)); }

surface_ptr_t surface_t::create(texture_ptr_t texture) { return surface_ptr_t(new surface_t(texture->width(), texture->height(), texture)); }

surface_ptr_t surface_t::create(const string &image_path) { return create(image_t::create(image_path)); }
...
void surface_t::memory_unlock(buffer_t &data) { _texture->set_pixels(data); }
